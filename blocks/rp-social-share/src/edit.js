...
export default function Edit({ attributes, setAttributes }) {
	const {
		...
		siteObject = useSelect((select) => {
			return select('core').getSite();
		}, []),
		currentPost = useSelect((select) => {
			return select('core/editor').getCurrentPost();
		}, []);

	useEffect(() => {
		const link = currentPost.link,
			title = currentPost.title;
		setAttributes({ postURL: link });
		setAttributes({ postTitle: title });
	}, [currentPost]);
	...
	return (
		<div {...useBlockProps({
			...
			<InspectorControls>
				...
				<PanelBody title={_x('Social Platforms', 'Inspector panel title', 'social-share')} initialOpen={true}>
					<ToggleControl
						label={_x('X (Twitter)', 'Toggle control label', 'social-share')}
						checked={enabledPlatforms.twitter}
						onChange={() => togglePlatform('twitter')}
					/>
					<ToggleControl
						label={_x('Facebook', 'Toggle control label', 'social-share')}
						checked={enabledPlatforms.facebook}
						onChange={() => togglePlatform('facebook')}
					/>
					<ToggleControl
						label={_x('LinkedIn', 'Toggle control label', 'social-share')}
						checked={enabledPlatforms.linkedin}
						onChange={() => togglePlatform('linkedin')}
					/>
					<ToggleControl
						label={_x('Pinterest', 'Toggle control label', 'social-share')}
						checked={enabledPlatforms.pinterest}
						onChange={() => togglePlatform('pinterest')}
					/>
					<ToggleControl
						label={_x('Email', 'Toggle control label', 'social-share')}
						checked={enabledPlatforms.email}
